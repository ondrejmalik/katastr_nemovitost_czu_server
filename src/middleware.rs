...
pub async fn track_latency(state: AppState, req: Request, next: Next) -> impl IntoResponse {
    ...
    if !state.no_print {
        println!("Request: {} {} took {:?}", method, uri, duration);
    }
...
pub async fn require_auth_cookie(state: AppState, req: Request, next: Next) -> impl IntoResponse {
    ...
    let session_id = match cookie_value {
        ...
        Some(v) => {
            ...
            if !state.no_print {
                ...
                println!("Auth middleware: found cookie katastr_session={v}");
    ...
    let is_valid = {
        ...
        let sessions = match state.sessions.read() {
            ...
            Err(_) => return (StatusCode::INTERNAL_SERVER_ERROR, "Lock error".to_string()).into_response(),
    ...
    if is_valid {
        ...
        if !state.no_print {
             ...
             println!("Auth middleware: valid session");
        ...
        if !state.no_print {
            ...
            println!("Auth middleware: invalid session - returning 401");
